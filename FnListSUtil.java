...
//
	...
	*/
    ...
    // my version of insertsort
	public static<T>
	FnList<T> insertSort(FnList<T> xs, ToIntBiFunction<T,T> cmp) {
    FnList<T> val0 = nil();
    while (!nilq(xs)) {
        T val1 =   xs.hd();   //sortedd
        xs = xs.tl();
        FnList<T> val2 =  nil();
        FnList<T> val3 = val0;
        while (!nilq(val3) && cmp.applyAsInt(val3.hd(), val1) <= 0)  {
            val2 =  cons(val3.hd(), val2);
            val3 = val3.tl(); }
        FnList<T> val4 = cons(val1, val3);
        while (!nilq(val2))  {
             val4 =  cons(val2.hd(), val4);
            val2 = val2.tl();  }
        val0 = val4; } return val0; } 
...
//
    ...
    FnList<T> result =  xs;
     while (wt < n) {
        FnList<T> val0 = nil();
        FnList<T> val1 =  result;
        while (!nilq(val1)) {
            Split<T> val2 = takeK(val1, wt);
            Split<T> val3 = takeK(val2.b, wt);
            FnList<T> val4 =  merge2(val2.a, val3.a, cmp);
            val0 =  prependRun(val0, val4);
            val1 =   val3.b; }
        result = reverse(val0);
         wt <<= 1; } return result; }
...
//herpers
private static<T>
Split<T> takeK(FnList<T> val0, int val1) {
    if (val1 <= 0 || nilq(val0)) return new Split<>(nil(), val0);
    FnList<T> x = nil();
    int y = 0;
    while (y < val1 && !nilq(val0)) {
        x = cons(val0.hd(), x);
        val0 = val0.tl();
        y += 1;
    }
    return new Split<>(reverse(x), val0);
...
private static<T>
FnList<T> merge2(FnList<T> a, FnList<T> b, ToIntBiFunction<T,T> val0) {
    FnList<T> val1 =  nil();
    while (!nilq(a) && !nilq(b)) {
         T x = a.hd(), y = b.hd();
        if (val0.applyAsInt(x, y) <= 0) {
            val1 = cons(x, val1);
            a =  a.tl(); } 
			 else {
            val1 = cons(y, val1);
...
private static<T>
FnList<T> prependRun(FnList<T> val0, FnList<T> val1) {
    FnList<T> x =  reverse(val1);
     while  (!nilq(x)) {
        val0 =  cons(x.hd(), val0);
        x = x.tl(); } return val0;  }
...
private static final class Split<T> {
    final FnList<T> a;
     final FnList<T> b;
    Split(FnList<T> val0,  FnList<T> val1) {
        this.a = val0;
         this.b =  val1; }  }
